...
public class EmailController {
	...
	public String listEmailHistory(@RequestParam Map<String, Object> param) {
		...
		ArrayList<SrvyEmailHist> srvyEmailHistList = srvyEmailHistService
				.getEmailHistList(this.getDatatableFilter(param), this.getDatatableSorting(param), start, length);
	...
	public String sendReminderToAll(@RequestParam Map<String, Object> param) {
		...
		Map<String, String> map = new HashMap<String, String>();
		SrvyPptService srvyPptService = (SrvyPptService) context.getBean("srvyPptService");
		ArrayList<SrvyPpt> srvyPptList = srvyPptService
				.getSrvyPptListBySrvyRecId(Integer.parseInt((String) param.get("srvyRecId")));
		for (SrvyPpt srvyPpt : srvyPptList) {
	...
	private void sendEmailManually(Map<String, Object> param, ArrayList<SrvyPpt> srvyPptList, String emailType) {
		...
		SrvyEmailHistService srvyEmailHistService = (SrvyEmailHistService) context.getBean("srvyEmailHistService");
		SendEmailService sendEmailService = (SendEmailService) context.getBean("sendEmailService");
		SrvyEmailTmpl srvyEmailTmpl = srvyEmailTmplService
				.getEmailTmpl(Integer.parseInt((String) param.get("srvyRecId")), emailType);

		SrvyRec srvyRec = srvyRecService.getSrvyRecByID(Integer.parseInt((String) param.get("srvyRecId")));

		if (srvyEmailTmpl != null) {
			for (SrvyPpt srvyPpt : srvyPptList) {
	...
	private SrvyEmailHist getDatatableFilter(Map<String, Object> param) {
		...
		try {
			do {
				try {
					if ("srvyRecId".equals(param.get("filter[" + i + "][name]"))
				...
				try {
					if ("srvyEmailHist.emailSendDateFrom".equals(param.get("filter[" + i + "][name]"))
							&& !"".equals(param.get("filter[" + i + "][value]"))) {
						srvyEmailHist.setEmailSendDateFrom(new Timestamp(Record.getDatetimeFormat()
								.parse((String) param.get("filter[" + i + "][value]") + " 00:00:00").getTime()));
					}
				} catch (Exception ex) {
				}
				try {
					if ("srvyEmailHist.emailSendDateTo".equals(param.get("filter[" + i + "][name]"))
							&& !"".equals(param.get("filter[" + i + "][value]"))) {
						srvyEmailHist.setEmailSendDateTo(new Timestamp(Record.getDatetimeFormat()
								.parse((String) param.get("filter[" + i + "][value]") + " 23:59:59").getTime()));
					}
				} catch (Exception ex) {
				}
				if ("srvyEmailHist.srvyEmailType.srvyEmailTypeId".equals(param.get("filter[" + i + "][name]"))
						&& !"".equals(param.get("filter[" + i + "][value]"))) {
					srvyEmailHist.getSrvyEmailType().setSrvyEmailTypeId((String) param.get("filter[" + i + "][value]"));
				}
				if ("srvyEmailHist.emailTo".equals(param.get("filter[" + i + "][name]"))
						&& !"".equals(param.get("filter[" + i + "][value]"))) {
					srvyEmailHist.setEmailTo((String) param.get("filter[" + i + "][value]"));
				}
				if ("srvyEmailHist.srvyEmailSts.srvyEmailStsId".equals(param.get("filter[" + i + "][name]"))
						&& !"".equals(param.get("filter[" + i + "][value]"))) {
